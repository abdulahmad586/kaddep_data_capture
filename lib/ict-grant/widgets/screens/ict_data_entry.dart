...
class ICTDataEntryPage extends StatelessWidget {
  final int? existingRecordId;
  ICTDataEntryPage({this.existingRecordId, super.key});
  ...
  Future<Object?> fetchData(ICTDataField field)async{
    return await cubitContext?.read<ICTDataEntryCubit>().fetchData(field);
  }
  ...
  Future<bool> proceedToNextStep(BuildContext c,int step)async {
    if (cubitContext != null) {
      switch (step) {
        case 1:
          if (await fetchData(ICTDataField.ownerPassportPhotoUrl) == null) {
            if(c.mounted)reportValidationError(c, "Picture of business owner is required");
            return false;
          } else if (await fetchData(ICTDataField.idDocPhotoUrl) == null) {
            if(c.mounted)reportValidationError(c, "Identity verification document is required");
            return false;
          }
          break;
        case 2:
          if (await fetchData(ICTDataField.ownerAtBusinessPhotoUrl) == null) {
            if(c.mounted)reportValidationError(c, "Picture of owner at business location is required");
            return false;
          }else if (await fetchData(ICTDataField.longitude) == null || await fetchData(ICTDataField.longitude) == 0) {
            if(c.mounted) reportValidationError(c, "Missing GPS coordinates, tap location panel for more info");
            return false;
          }
          break;
        case 3:
          if (await fetchData(ICTDataField.certPhotoUrl) == null) {
            if(c.mounted)reportValidationError(c, "Business registration certificate image is required");
            return false;
          }else if ((await fetchData(ICTDataField.businessRegIssuer)) == "CAC" && await fetchData(ICTDataField.cacProofDocPhotoUrl) == null) {
            if(c.mounted) reportValidationError(c, "CAC proof of ownership page image is required");
            return false;
          }
          break;
        case 4:
          final itemsPurchasedJson = await fetchData(ICTDataField.itemsPurchased) as String?;
          ...
          if ((await fetchData(ICTDataField.ictProcCat)) == "Salary" && ((await fetchData(ICTDataField.renumerationPhotoUrl) == null) || (await fetchData(ICTDataField.groupPhotoUrl)) == null) ) {
            if(c.mounted)reportValidationError(c, "Salary remuneration (payroll) & Staff group images are required");
            return false;
          }else if ((await fetchData(ICTDataField.ictProcCat)) != "Salary" &&  ( await fetchData(ICTDataField.itemsPurchased) == null || itemsPurchased.isEmpty ) ) {
            if(c.mounted) reportValidationError(c, "Please add at least one item you purchased");
            return false;
          }
          break;
        case 5:
          final taxIssuer = (await fetchData(ICTDataField.issuer)) as String?;
          if ( (taxIssuer != null && taxIssuer.isNotEmpty) && (await fetchData(ICTDataField.taxRegPhotoUrl)) ==null ) {
  ...
  Widget build(BuildContext context) {
    return PopScope(
      onPopInvoked:(popped)async{
        await cubitContext?.read<ICTDataEntryCubit>().onEnd();
      },
      child: BlocProvider(
        create: (_) => ICTDataEntryCubit(ICTDataEntryState(page: 1, regId: existingRecordId)),
        child: BlocBuilder<ICTDataEntryCubit, ICTDataEntryState>(
                              ...
                              builder: (c) {
                                ...
                                switch (activeStep) {
                                  ...
                                  case 3:
                                    ...
                                    break;
                                  case 4:
                                    widget = ICTProcurementExpensesFragment(cubitContext: context, state: state, formKey: formKeys[4],);
                                    break;
                                  case 5:
            ...
            child: Row(
              ...
              children: [
                AppTextButton(
                    ...
                    onPressed: () {
                      if (activeStep != 0) {
                        cubitContext?.read<ICTDataEntryCubit>().prevPage();
                      }
                    }),
                      ...
                      onPressed: () async {
                        if (activeStep < stepsLabel.length - 1) {
                          if (await proceedToNextStep(context,activeStep)) {
                            cubitContext?.read<ICTDataEntryCubit>().nextPage(stepsLabel.length);
                            cubitContext?.read<ICTDataEntryCubit>().switchPage(activeStep);
                          }
                        } else {
  ...
  Widget _buildFinishPage(BuildContext context, ICTDataEntryState state) {

    int activeStep = (state.page ??1)-1;
    return Center(
      child: FutureBuilder(
          future: context.read<ICTDataEntryCubit>().switchPage(activeStep+1),
          builder: (c, snapshot) {
            return Column(
              children: [
                Card(
